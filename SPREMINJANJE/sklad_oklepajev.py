...
# 
# V tej nalogi bomo sestavili funkcijo, ki bo preverila, ali so v danem
# nizu oklepaji pravilno gnezdeni. Pri tem bomo uporabili pomožen seznam,
# v katerega ob sprehodu po nizu dodajamo oziroma ostranjujemo oklepaje.
# Natančneje, ko vidimo znak `(`, ga dodamo v pomožen seznam. Ko vidimo znak
# `)`, mora pa zadnji element v pomožnem seznamu biti `(`, saj zapiramo nek 
# oklepaj. V tem primeru zadnji element pomožnega seznama odstranimo.
# =====================================================================@009899=
# 1. podnaloga
# Sestavite funkcijo `oklepaji(niz)`, ki bo preverila, ali so v danem
# nizu oklepaji pravilno gnezdeni. 
...
def oklepaji(niz):
    ...
    for znak in niz:
        if znak == '(':
            sklad.append('(') #odpremo oklepaj
        elif znak == ')':
            if sklad == []:
                return False # ne moremo zapreti, ker ni nič odprtega
            else:
                sklad.pop() #zapremo oklepaj
                # ali sklad = sklad[:-1] ;sklad je vse brez zadnjega člena
    return sklad == [] #preverimo, če je kakšen oklepaj še odprt
# =====================================================================@009938=
# 2. podnaloga
# Sestavite funkcijo `razlicni_oklepaji(niz)`, ki bo preverila, ali so v danem
# nizu oklepaji pravilno gnezdeni. Pri tem upoštevajte, da `niz` lahko
# vsebuje poleg oklepajev `()` še para `{}` in `[]`.
# Na ostale znake naj se funkcija ne ozira. Zgled:
# 
#     >>> oklepaji('(a + b)^2 = ([{a^2} + 2ab] + b^2)')
#     True
    ...
    for znak in niz:
        if znak in ['(', '{', '[']:
            ...
            if sklad == []:
                ...
                zadnji_oklepaj = sklad.pop()
                if (zadnji_oklepaj + znak) not in ['()', '[]', '{}']:
                    #zadnji oklepaj in znak se ne ujemata!
                    return False
    return sklad == []
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            Check.equal("razlicni_oklepaji('(a + b)^2 = ([{a^2} + 2ab] + b^2)')", True) and \
            Check.equal("razlicni_oklepaji('(]){()')", False) and \
            Check.equal('razlicni_oklepaji("[2 + a] + {[3 - x] + (3 - 2)}")', True) and \
            Check.equal('razlicni_oklepaji("{} +krneki-/*+3@ (() - ()((()())))")', True) and \
            Check.equal('razlicni_oklepaji("((({}) + (")', False) and \
            Check.equal('razlicni_oklepaji("[2 + 3] - 3 + ) {")', False)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
