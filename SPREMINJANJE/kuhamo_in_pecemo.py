...
def pomnozi(recept, faktor):
    pomnozen = {}
    for kljuc in recept:
        pomnozen[kljuc] = recept[kljuc] * faktor
    #lahko bi tudi:
    #for kljuc, vrednost in recept.items():
        #pomnozen[kljuc] = vrednost * faktor
    return pomnozen
...
def ali_imamo_sestavine(recept, shramba):
    for sestavina in recept:
        if recept[sestavina] > shramba.get(sestavina, 0):
            return False
    return True # če pridemo čez celo for zanko, je vse v redu!
# =====================================================================@009937=
# 3. podnaloga
...
def kaj_moramo_se_kupiti(recept, shramba):
    moramo_kupiti = {}
    for sestavina, vrednost in recept.items():
        v_shrambi = shramba.get(sestavina, 0)
        if vrednost > v_shrambi:
            moramo_kupiti[sestavina] = vrednost - v_shrambi
    return moramo_kupiti
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            ...
            Check.equal("pomnozi({'jajca': 3, 'moka': 500, 'sol': 200, 'olje': 3, 'kakav': 350}, 3.5)", {'jajca': 10.5, 'sol': 700, 'moka': 1750, 'kakav': 1225, 'olje': 10.5})
        except:
    ...
    if Check.part():
        try:
            ...
            Check.equal("kaj_moramo_se_kupiti({'jajca': 3, 'moka': 500}, {'moka': 1000, 'jajca': 6, 'sladkor': 1000, 'grah': 7})", {})
            Check.equal("kaj_moramo_se_kupiti({'jajca': 3, 'moka': 500, 'grah': 14}, {'moka': 1000, 'jajca': 6, 'sladkor': 1000, 'grah': 7})", {'grah': 7})
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
