...
def razpolovi(l):
    dolzina = len(l)
    polovica_dolzine = dolzina // 2
    return (l[0:polovica_dolzine], l[polovica_dolzine:])
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            Check.equal('razpolovi([])', ([],[]))
            Check.equal('razpolovi([1,2])', ([1], [2]))
            Check.equal('razpolovi(["a", "b", "c"])', (["a"], ["b", "c"]))
            Check.equal('razpolovi(["a", "b", "c", "d"])', (["a", "b"], ["c", "d"]))
        except:
    ...
    if Check.part():
        try:
            Check.equal('zamenjaj([1,2,3,4], 1, 2)', [1,3,2,4])
            Check.equal('zamenjaj([1,2,3,4], 1, 2017)', [1,2,3,4])
            Check.equal('zamenjaj([1,2,3,4], 2, 1)', [1,3,2,4])
            Check.equal('zamenjaj([1,2,3,4], 2017, 1)', [1,2,3,4])
            Check.equal('zamenjaj([1,2,3,4], 2, 2)', [1,2,3,4])
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
    ...
    if Check.part():
        try:
            Check.equal('porezi([])', [[]])
            Check.equal('porezi([1,2])', [[1,2], [2], []])
            Check.equal('porezi([1,2,3,4])', [[1,2,3,4], [2,3,4], [3,4], [4], []])
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
